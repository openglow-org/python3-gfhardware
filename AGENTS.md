...
## python3-gfhardware repository rules

This repository provides `gfhardware`, the Python hardware library for the
machine, with its `_cam` capture extension. In `forgefirm-app/` it also
carries the cloud-mode applications the image installs: `gfcloud.py` (the
cloud-mode controller daemon), `gfhome.py` (one-shot homing through the
Glowforge service) and `ffmachine.py` (the glue both use). Those
applications are not part of the `gfhardware` package.
...
- **The licensing is mixed, and it must stay declared.**
  `gfhardware/src/bayer.c` and `bayer.h` are LGPL-2.1-or-later from
  libdc1394; the rest is MIT. The package declares
  `MIT AND LGPL-2.1-or-later`, ships both license texts, and states the
  rebuild and relink offer in its README. Do not blur that.
- **The cameras capture only with the lid closed.** The direct-V4L2 fallback
  in `cam.capture()` enforces the same rule the daemon does, and it fails
  closed. There is no setting to disable it.
- **The client is a cooling-engine client, not a thermal writer.** It reports
  its job state and enforces the published verdict. It writes the run fan
  duties directly in exactly one case: the engine is provably gone while the
  laser is armed.
